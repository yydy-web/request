...
[![Release](https://img.shields.io/github/v/release/yydy-web/request?label=)](https://github.com/yydy-web/request/releases)

面向链式调用的 HTTP 客户端封装。底层支持 **Axios** 与 **Fetch** 两种传输方式，提供统一的 `Request` API（路径模板、`GET` 缓存、重复请求取消、简单并发队列等）。

正式发布在 **GitHub Packages**（npm 协议），CI 使用仓库自带的 `GITHUB_TOKEN` 发版，无需配置 `NPM_TOKEN`。

## 特性

- **双入口**：`@yydy-web/request`（Axios）与 `@yydy-web/request/fetch`（Fetch）
...
包作用域必须与 GitHub 仓库所有者一致（`@yydy-web`）。从 GitHub Packages 安装前，在项目或用户目录的 `.npmrc` 中指向注册表（本仓库根目录已包含 `@yydy-web:registry=…`，可复制到业务项目）：

```ini
@yydy-web:registry=https://npm.pkg.github.com
//npm.pkg.github.com/:_authToken=YOUR_GITHUB_PAT
```

`YOUR_GITHUB_PAT` 建议使用具备 **`read:packages`** 的 [Personal Access Token](https://github.com/settings/tokens)（仅读公开包时也可按组织策略使用 `GITHUB_TOKEN` 于 CI）。

```bash
pnpm add @yydy-web/request
# 使用 Axios 传输时再安装
pnpm add axios
```
...
```ts
import { RequestFactory, setRequest } from '@yydy-web/request'
...
```ts
import { clearStore, getStore, setStore } from '@yydy-web/request'
```
...
子路径 **`@yydy-web/request/fetch`** 使用原生 `fetch` 传输，不依赖 Axios：
...
| 入口 | 用途 |
|------|------|
| `@yydy-web/request` | `RequestFactory`、`Request`、`getStore` / `setStore` / `clearStore`、`getRequest` / `setRequest`、类型等 |
| `@yydy-web/request/fetch` | `createFetchRequest`、`Request`、传输相关类型 |
